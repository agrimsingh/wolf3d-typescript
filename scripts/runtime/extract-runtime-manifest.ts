...
async function main(): Promise<void> {
  ...
  const traceSymbolMap = new Map<number, RuntimeTraceSymbolMetadata>(traceSymbols.map((entry) => [entry.id, entry]));
  ...
  const requiredSymbols: RuntimeTraceSymbolMetadata[] = hitIds.map((id) => {
    const known = traceSymbolMap.get(id);
    if (known) {
      return known;
    }
  ...
  const excludedSymbols = traceSymbols.filter((entry) => !allHits.has(entry.id));
    ...
    `${JSON.stringify(
      {
        ...
        traceSymbolIndex: traceSymbols.map((entry) => ({
          id: entry.id,
